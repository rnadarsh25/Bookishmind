...
<?php
    ...
    if(isset($_POST['create_record'])){
        ...
        if(empty($record_new_title)){
            ...
            $query = "INSERT INTO views VALUES('', '$record_title', 'Novel', 0)";
        ...
        $query = "INSERT INTO records VALUES('', 1,$record_genre_id, '$record_title', '$record_series_name', '$record_index','$record_author','$record_content','$record_image','$record_rate_star','$record_rate_cmt','$record_summary','$record_status', now(), $user_id)";
        ...
        echo "<div class=' form-group bg-success'>New record Created !  <a href='../novels.php?p_id=$the_record_id'>View Novel</a></div>";
...
<form action="" method="post" enctype="multipart/form-data">
    ...
    <div class="form-group" id="new_title" style="display:none">
        <label for="record_new_title">New Novel Title</label>
        <input type="text" name="record_new_title" class="form-control">
    </div>
    
    <div class="form-group" id="old_title">
        <label for="">Previous Novel Titles</label>
        <select name="record_title" class="form-control" id="">
            <?php
                $query = "SELECT DISTINCT * FROM records WHERE record_category_id=1 GROUP BY record_title ORDER BY record_id DESC";
    ...
    <div class="form-group" id="new_series" style="display:none">
        <label for="record_new_series_name">New Novel Series</label>
        <input type="text" name="record_new_series_name" class="form-control">
    </div>

    <div class="form-group" id="old_series">
        <label for="">Previous Novel Series</label>
        <select name="record_series_name" class="form-control" id="">
            <?php
                $query = "SELECT DISTINCT * FROM records WHERE record_category_id=1 GROUP BY record_series_name ORDER BY record_id DESC";
    ...
    <div class="form-group">
        <label for="record_index">Novel Index</label>
        <input type="text" name="record_index" class="form-control">
    </div>

    <div class="form-group">
        <label for="record_author">Novel Author</label>
        <input type="text" name="record_author" class="form-control">
    </div>
    
    <div class="form-group">
        <label for="">Novel Status</label>
    ...
    <div class="form-group">
        <label for="record_rate_star">Novel Star</label>
        <input type="text" name="record_rate_star" class="form-control" placeholder="Write only one time">
    </div>
    <div class="form-group">
        <label for="record_rate_cmt">Novel Comment</label>
        <input type="text" name="record_rate_cmt" class="form-control" placeholder="Write only one time">
    </div>
    
    <div class="form-group">
        <label for="record_image">Novel Image</label>
        <input type="file" name="record_image" class="form-control">
    </div>
    <div class="form-group">
        <label for="record_summary">Novel Summary</label><br>
        <textarea name="record_summary" placeholder="Write only one time" class="form-control" rows="10"></textarea>
    </div>

    <div class="form-group">
        <label for="record_content">Novel Content</label><br>
        <textarea name="record_content" id="body" class="form-control" rows="10"></textarea>
    </div>
    
    <div class="form-group">
        <input type="submit" name="create_record" value="Create New Novel"  class="btn btn-success btn-md">
    </div>
